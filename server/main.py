...
firebase_admin.initialize_app(
    cred,
    {
        "databaseURL": "https://mes-app-9c6f4-default-rtdb.asia-southeast1.firebasedatabase.app/"
    },
)
...
async def post_user(user_data: UserData):
    ref = db.reference("users")
    ref.push({"username": user_data.username, "password": user_data.password})
...
async def insert_qr_code_data(qr_code_data):
    ...
    heat_no_exists = (
        ref.order_by_child("heat_no").equal_to(qr_code_data["heat_no"]).get()
    )

    if heat_no_exists:
        raise HTTPException(
            status_code=400, detail="Heat No already exists in the database"
        )
...
async def post_mill_data(mill_data: MillData):
    ref = db.reference("mill_data")
    ref.push(
        {"millname": mill_data.millname, "production_today": mill_data.production_today}
    )
